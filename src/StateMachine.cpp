...
StateMachine::StateMachine(int input_length, double tolerance, int min_pixels, double aspect) {
...
bool StateMachine::in_bounds(int count, double scale) {
...
#define IN_BOUNDS3(x) this->in_bounds(this->pixel_counts[x], 3.0)
...
bool StateMachine::valid_transition(int x, bool value) {
    switch(this->state) {
        case 0:
            return (!value && x != 0);
        case 1:
            this->min_bound = (this->aspect * ((double)this->pixel_counts[1] * this->tolerance));
            this->max_bound = (this->aspect * ((double)this->pixel_counts[1] / this->tolerance));
...
Marker* StateMachine::step(int x, int y, bool value) {
    ...
    this->pixel_counts[this->state] += 1;
    if(this->state == 1) {
        if(this->pixel_counts[1] > this->max_pixels) {
            this->reset(y, value);
        }
    } else if(this->state > 1) {
        double max_bound = this->max_bound;
        if(this->state == 3) {
            max_bound *= 3.0;
        }
